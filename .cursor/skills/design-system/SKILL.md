...
### Chat (`inbox.tsx`)
Sin avatares. Entrante: `bg-gris-wash text-ink rounded-2xl`. Saliente: `bg-ink text-white`, alineado a la derecha. Sistema: `bg-rojo-wash text-rojo`. Timestamp 10px muted. Composer: textarea + `Enviar` primario (Enter envía, Shift+Enter salto). Toast con componente `Toast` del kit. Notas de voz: player propio (`AudioNote`: play/pausa + barra). Hilo con `flex-col-reverse` (scrollTop 0 = abajo, sin pegar con JS al abrir). Botón “ir abajo” si el usuario subió. Poll: ~2.5s con chat abierto, ~6s en lista; pausa si la pestaña está oculta.
