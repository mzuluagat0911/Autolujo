...
### Chat (`inbox.tsx`)
Sin avatares. Entrante: `bg-gris-wash text-ink rounded-2xl`. Saliente: `bg-ink text-white`, alineado a la derecha. Sistema: `bg-rojo-wash text-rojo`. Timestamp 10px muted. Composer: textarea + `Enviar` primario. Toast con componente `Toast` del kit. Notas de voz: player propio (`AudioNote`: play/pausa + barra), no `<audio controls>` nativo. Scroll: stick al final solo si el usuario ya estaba abajo; botón flotante “ir abajo” si subió. Poll: ~2.5s con chat abierto, ~6s en lista; pausa si la pestaña está oculta.
